...
graph LR
    ...
    C1 -.->|ITextToSpeechService 追加実装| F2
```
...
| **新TTSエンジン追加** | `ITextToSpeechService` 実装クラスを追加・`TtsEngine` enum に値を追加 | `AppSetup.cs` + 新クライアントクラス |
| **会話履歴** | `ExternalLanguageModelClient` にメッセージリストを追加 | `ExternalLanguageModelClient` のみ |
| **処理キャンセル** | インターフェースに `CancellationToken` を追加し `_cts.Token` を渡す | 全サービスインターフェース + 各クライアント |
| **クラウド設定** | `IConfigProvider` 実装クラスを追加 | `AppSetup.cs` の1行 |
| **中間APIサーバー** | 各Clientの送信先URLを変更 | 各Clientの定数1行 |
| **音声ストリーミング** | TTS生成しながら再生する仕組みを追加 | `ITextToSpeechService` + 各TTSクライアント |
