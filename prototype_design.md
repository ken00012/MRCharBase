...
| XR UI | Meta XR Interaction SDK（PokeInteractable） |
...
### 10.8 XRInteractionUI と UserSubtitleUI

**ボタン実装方針**: Meta XR Interaction SDK の **`PokeInteractable`** を使用します（指でタッチして押す操作）。
...
World Space Canvas
  ...
  └─ UserSubtitle (GameObject)    ← XRInteractionUI とは独立した GameObject
       └─ UserSubtitleUI (MonoBehaviour)
```

> [!TIP]
> `OnRecordButtonTapped()` は引数なしのメソッドです。  
> `InteractableUnityEventWrapper` の UnityEvent から **Inspector のみ**で接続できます。

```csharp
// XRInteractionUI.cs
// 責務: AI字幕 / エラー / 状態インジケーター
// World Space Canvas に配置する（Meta XR Interaction SDK対応）
public class XRInteractionUI : MonoBehaviour
{
    [SerializeField] private TMP_Text subtitleText;
    [SerializeField] private Image    recordButtonBackground;
    [SerializeField] private TMP_Text buttonLabel;
    [SerializeField] private Color normalColor    = Color.white;
    [SerializeField] private Color errorColor     = Color.red;
    [SerializeField] private Color idleColor      = new Color(0.86f, 0.23f, 0.23f); // 赤
    [SerializeField] private Color listeningColor = new Color(0.23f, 0.72f, 0.34f); // 緑
...
| `XRInteractionUI` | MonoBehaviour | AI字幕 / エラー表示 / ボタン状態色（ボタンイベント発火元は `InteractableUnityEventWrapper`） |
| `UserSubtitleUI` | MonoBehaviour | ユーザー発話字幕表示（STT 結果をシアン色で表示・XRInteractionUI とは独立） |
...
- **XRInteractionUI (Record Button)**
  - [ ] `InteractableUnityEventWrapper` の `WhenSelect()` : CharacterStateController の `OnRecordButtonTapped` に接続
  - [ ] `subtitleText`: AI字幕用 TextMeshPro をアタッチ
  - [ ] `recordButtonBackground`: `RecordButton > ButtonBackground` の Image をアタッチ
...
- **UserSubtitleUI GameObject**
  ...
  - [ ] `userTextColor`: デフォルトはシアン（AIの字幕・白と区別）

---

*設計書バージョン: 5.0 | 更新日: 2026-04-22*
