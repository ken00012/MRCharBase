...
| TTS | ElevenLabs / Fish Audio / Google Cloud TTS / OpenAI TTS（切り替え可能） |
...
> [!NOTE]
> `fishAudioModel` を空文字にすると `ExternalFishAudioClient` 内で `"s2-pro"` にフォールバックします。  
> `googleTtsLanguageCode` / `googleTtsVoiceName` を空文字にするとそれぞれ `"ja-JP"` / `"ja-JP-Neural2-B"` にフォールバックします。  
> 使用しないTTSエンジンのフィールドは省略可能です。
...
`ExternalElevenLabsClient.SynthesizeAsync()` は `AudioClip` を直接返します。  
...
```csharp
// ExternalElevenLabsClient.cs の実装イメージ
...
│   │   │   └── UserSubtitleUI.cs         ← ユーザー発話字幕（STT結果）
│   │   └── Character/                   ← キャラクター表示
...
| `UserSubtitleUI` | MonoBehaviour | ユーザー発話字幕表示（STT 結果を `CharacterStateController` から受け取り表示） |
...
| `ExternalElevenLabsClient` | Pure C# | TTS API通信 ElevenLabs（AudioClip返却・`TtsEngine.ElevenLabs` 選択時に使用） |
| `ExternalFishAudioClient` | Pure C# | TTS API通信 Fish Audio（AudioClip返却・`TtsEngine.FishAudio` 選択時に使用） |
| `ExternalOpenAITtsClient` | Pure C# | TTS API通信 OpenAI（AudioClip返却・`TtsEngine.OpenAI` 選択時に使用） |
| `ExternalGoogleTtsClient` | Pure C# | TTS API通信 Google Cloud TTS（Base64 MP3→一時ファイル→AudioClip・`TtsEngine.Google` 選択時に使用） |
...
- [ ] `ExternalElevenLabsClient.cs` 実装・Editor疎通確認
- [ ] Editorでエンドツーエンド動作確認
...
| **音声ストリーミング** | TTS生成しながら再生する仕組みを `ITextToSpeechService` に追加 | `ExternalElevenLabsClient` |
...
- **CharacterRoot GameObject (CharacterStateController)**
  ...
  - [ ] `userSubtitleUI`: シーン内の UserSubtitleUI をアタッチ
