...
AIが回答を生成し、先生の声でUnityが音声を再生するシステム。
...
| TTS | ElevenLabs / Fish Audio / Google Cloud TTS / OpenAI TTS（AppSetup.TtsEngine で切り替え可能） |
...
| `UserSubtitleUI` | World Space Canvas（XRInteractionUI とは独立したGameObject） |
...
stateDiagram-v2
    ...
    Listening --> Thinking : 再度ボタン押下（録音完了）
...
|---|---|---|---|
| **Idle** | 待機中 | — | 通常表示 |
| **Listening** | 録音中 | 「録音中...もう一度タップで送信」 | （将来: 録音中エフェクト） |
| **Thinking** | AI処理中 | **「AIが考えています...」** | （将来: 考え中アニメーション） |
| **Speaking** | 話し中 | 回答字幕（XRInteractionUI）<br>ユーザー発話字幕（UserSubtitleUI）表示中 | （将来: リップシンク） |
...
sequenceDiagram
    ...
    participant USUI as UserSubtitleUI
    ...
    U->>UI: Button タップ (Ray/Poke)（1回目）
    UI->>CSC: OnRecordButtonTapped()
    CSC->>USUI: Clear()（前回のSTT結果をリセット）
    CSC->>VIH: StartRecording()
    CSC->>CSC: 状態 → Listening

    U->>UI: Button タップ (Ray/Poke)（2回目）
    UI->>CSC: OnRecordButtonTapped()
    ...
    alt STT成功
        STT-->>CSC: 質問テキスト
        CSC->>USUI: ShowUserSubtitle(question)
    else STT失敗
        STT-->>CSC: 例外
...
| フィールド | 対象 TTS | 説明 |
|---|---|---|
| `openAIApiKey` | STT / LLM / OpenAI TTS 共用 | OpenAI API キー |
| `elevenLabsApiKey` | ElevenLabs | ElevenLabs API キー |
| `elevenLabsVoiceId` | ElevenLabs | 使用する音声 ID |
| `fishAudioApiKey` | Fish Audio | Fish Audio API キー |
| `fishAudioReferenceId` | Fish Audio | 参照音声 ID |
| `fishAudioModel` | Fish Audio | 空文字の場合は `"s2-pro"` にフォールバック |
| `googleTtsApiKey` | Google Cloud TTS | Google Cloud API キー |
| `googleTtsLanguageCode` | Google Cloud TTS | 空文字の場合は `"ja-JP"` にフォールバック |
| `googleTtsVoiceName` | Google Cloud TTS | 空文字の場合は `"ja-JP-Neural2-B"` にフォールバック |
| `openAiTtsModel` | OpenAI TTS | 空文字の場合は `"tts-1"` にフォールバック |
| `openAiTtsVoice` | OpenAI TTS | 空文字の場合は `"alloy"` にフォールバック |
| `systemPrompt` | LLM | キャラクターの振る舞いを定義するシステムプロンプト |

> [!NOTE]
> 使用しない TTS エンジンのフィールドは省略可能です。  
> `AppSetup` の `TtsEngine` Inspector フィールドで使用するエンジンを選択してください。
...
graph LR
    ...
    D -->|回答テキスト| E["ExternalElevenLabsClient\nまたは各 TTS クライアント"]
    E -->|AudioClip| F["SpatialAudioPlayer Play"]
    F -->|Play() 空間音響| G["AudioSource 音声出力"]
    C -->|質問テキスト| H["UserSubtitleUI ShowUserSubtitle"]
    D -->|回答テキスト| I["XRInteractionUI ShowSubtitle"]
```

### 音声→AudioClip 変換

各 TTS クライアントの `SynthesizeAsync()` は `AudioClip` を直接返します。  
内部で `DownloadHandlerAudioClip` を明示的に設定することで MP3 を正しく受信します（ElevenLabs / Fish Audio / OpenAI TTS）。  
Google Cloud TTS のみ Base64 エンコードされた MP3 を JSON レスポンスで受信し、一時ファイルを経由して AudioClip に変換します。
...
```csharp
// ElevenLabs / Fish Audio / OpenAI TTS 共通実装パターン
...
| TTS受信（ElevenLabs / Fish Audio / OpenAI） | MP3 → AudioClip | `DownloadHandlerAudioClip` 明示設定が必須 |
| TTS受信（Google Cloud TTS） | Base64 MP3 → 一時ファイル → AudioClip | JSON レスポンスの `audioContent` フィールドに格納 |
...
| TTS失敗 | 「音声合成に失敗しました。\n [TTS] {エラー詳細}」 |
| 録音データなし | 「録音データが取得できませんでした。」 |
| config.json 読み込み失敗 | 「設定ファイルの読み込みに失敗しました。」 |
...
| XR UI | Meta XR Interaction SDK（RayInteractable / PokeInteractable / ColliderSurface） |
| VRMローダー | UniVRM **0.104.0**（Unity 6000.0.60f1 動作確認済み） |
| UI | Unity uGUI + TextMeshPro |
| STT | OpenAI Whisper API |
| LLM | OpenAI Chat Completions API（gpt-4o-mini 推奨） |
| TTS | ElevenLabs API / Fish Audio API / Google Cloud TTS API / OpenAI TTS API（`AppSetup.TtsEngine` で選択） |
| 通信 | UnityWebRequest |
...
> - **ElevenLabs / Fish Audio / OpenAI TTS**: バイナリ（MP3）レスポンスのためJSON解析不要  
> - **Google Cloud TTS**: `{"audioContent": "<base64>"}` → `[Serializable]` クラス経由で `audioContent` フィールドを取り出す
...
│   ├── Scripts/
│   │   ├── App/                          ← 起動・DI
│   │   │   ├── AppSetup.cs               ← Composition Root（TtsEngine enum 定義含む）
│   │   │   ├── LocalConfigProvider.cs    ← APIキー読込（IConfigProvider実装）
│   │   │   └── AppConfig.cs              ← config.jsonデータクラス（全TTSフィールド含む）
│   │   ├── Core/                         ← フロー制御
│   │   │   ├── CharacterStateController.cs  ← 状態管理・パイプライン・UserSubtitleUI参照
...
│   │   │   │   ├── ExternalElevenLabsClient.cs    ← ElevenLabs TTS
│   │   │   │   ├── ExternalFishAudioClient.cs     ← Fish Audio TTS
│   │   │   │   ├── ExternalOpenAiTtsClient.cs     ← OpenAI TTS
│   │   │   │   └── ExternalGoogleTtsClient.cs     ← Google Cloud TTS
...
│   │   ├── UI/                           ← XR UI
│   │   │   ├── XRInteractionUI.cs        ← AI字幕 / エラー / ボタン状態
│   │   │   └── UserSubtitleUI.cs         ← ユーザー発話字幕（STT結果・シアン色）
│   │   └── Character/                   ← キャラクター表示
...
{
    // ── OpenAI ──────────────────────────────────────────────────
    public string openAIApiKey;         // STT (Whisper) / LLM (GPT) / OpenAI TTS で共用
...
public class AppSetup : MonoBehaviour
{
    ...
    public enum TtsEngine { ElevenLabs, FishAudio, Google, OpenAI }
    ...
    private async UniTaskVoid InitAsync()
    {
        ...
        // TTS エンジン選択（useMock = true の場合は常に Mock）
        ITextToSpeechService tts;
        if (useMock)
        {
            tts = new MockTextToSpeechService();
        }
        else
        {
            tts = ttsEngine switch
            {
                TtsEngine.FishAudio => new ExternalFishAudioClient(config),
                TtsEngine.Google    => new ExternalGoogleTtsClient(config),
                TtsEngine.OpenAI    => new ExternalOpenAiTtsClient(config),
                _                   => new ExternalElevenLabsClient(config), // ElevenLabs（default）
            };
        }
...
> ダイアログで「許可」を押す前に録音ボタンを押してしまうと録音に失敗します。
...
```csharp
// CharacterStateController.cs
// 配置: キャラクター Root GameObject（Core/ フォルダ）
...
public class CharacterStateController : MonoBehaviour
{
    ...
    // uGUI Button の OnClick() イベントに接続 (PointableCanvasModule 経由)
    ...
    public void OnDebugInputFromMenu() => OnDebugInput(_debugText).Forget();
...
> [!TIP]
> **デバッグ用テキスト入力**: `CharacterStateController.OnDebugInput(text)` をInspectorの3点メニューから「Debug: OnDebugInput」を実行してください。
...
**ボタン実装方針**: Meta XR Interaction SDK の **`RayInteractable`** + **`PokeInteractable`** を用いた物理ベースのネイティブアプローチを採用します。
`PointableCanvasModule` および uGUI `Button` は使用しません。色の管理は `ButtonStateColorController` に委譲し、`XRInteractionUI` は字幕とエラー表示に特化します。

```text
シーン構成（UI階層）
EventSystem
  └─ Input System UI Input Module  ← PointableCanvasModule は使用しない

UICanvas (World Space Canvas, XRInteractionUI)
  ├─ Background / SubtitleText
  └─ RecordButton
       │  ├─ BoxCollider (isTrigger=true, size≒ボタン寸法×0.02m厚)
       │  ├─ ColliderSurface  → BoxCollider をアサイン
       │  ├─ PlaneSurface
       │  ├─ BoundsClipper
       │  ├─ ClippedPlaneSurface  → PlaneSurface + BoundsClipper をアサイン
       │  ├─ RayInteractable   → Surface: ColliderSurface
       │  ├─ PokeInteractable  → SurfacePatch: ClippedPlaneSurface
       │  ├─ InteractableUnityEventWrapper [Ray用]
       │  │    ├─ _interactableView: RayInteractable
       │  │    ├─ WhenHover   → ButtonStateColorController.ExternalHoverEnter()
       │  │    ├─ WhenUnhover → ButtonStateColorController.ExternalHoverExit()
       │  │    ├─ WhenSelect  → ButtonStateColorController.ExternalPressEnter()
       │  │    │               → CharacterStateController.OnRecordButtonTapped()
       │  │    └─ WhenUnselect→ ButtonStateColorController.ExternalPressExit()
       │  └─ InteractableUnityEventWrapper [Poke用]
       │       └─ （Ray用と同じイベント構成・_interactableView: PokeInteractable）
       ├─ ButtonBackground (RectTransform, Image, ButtonStateColorController)
       └─ ButtonLabel (RectTransform, TextMeshProUGUI)

UserSubtitleUICanvas (World Space Canvas, UserSubtitleUI)
  ├─ Background
  └─ UserSubtitleText (RectTransform, TextMeshProUGUI)
```

> [!TIP]
> `ClippedPlaneSurface` が `ISurfacePatch` を実装しており、`PokeInteractable.SurfacePatch` として指定します。  
> `ColliderSurface`（`ISurface` 実装）は `RayInteractable.Surface` 専用です。両者は型が異なります。

```csharp
// XRInteractionUI.cs
// 配置: UICanvas root GameObject
// 責務: AI字幕 / エラー / 状態表示。ボタン色は ButtonStateColorController に委譲する。
public class XRInteractionUI : MonoBehaviour
{
    [SerializeField] private TMP_Text                   subtitleText;
    [SerializeField] private TMP_Text                   buttonLabel;
    [SerializeField] private ButtonStateColorController _buttonColor;
...
// ButtonStateColorController.cs
// 配置: UICanvas/RecordButton/ButtonBackground
// 責務: ON/OFF状態色と Hover/Press 色を合成して Image.color に適用する
// 2つのモードをサポート:
//   ISDK ネイティブ: InteractableUnityEventWrapper から External* メソッドを呼び出す
//   EventSystem互換: IPointerXxxHandler 経由（PointableCanvasModule 使用時）
[RequireComponent(typeof(Image))]
public class ButtonStateColorController : MonoBehaviour,
    IPointerEnterHandler, IPointerExitHandler,
    IPointerDownHandler,  IPointerUpHandler
{
    public void SetState(bool isOn) { /* ON/OFF状態の切り替え */ }

    // ISDK ネイティブモード用 External API
    public void ExternalHoverEnter() { /* _isHovered = true; ApplyColor(); */ }
    public void ExternalHoverExit()  { /* _isHovered = false; ApplyColor(); */ }
    public void ExternalPressEnter() { /* _isPressed = true; ApplyColor(); */ }
    public void ExternalPressExit()  { /* _isPressed = false; ApplyColor(); */ }
}

// UserSubtitleUI.cs
// 責務: STT で取得したユーザー発話テキストの表示・クリアのみを担う（AI字幕は XRInteractionUI が担当）
// 配置: World Space Canvas 内の独立した UserSubtitle GameObject
public class UserSubtitleUI : MonoBehaviour
{
    [SerializeField] private TMP_Text userSubtitleText;
    [SerializeField] private Color    userTextColor = Color.cyan; // AIの字幕（白）と区別するためシアン
...
| スクリプト | 種別 | 責務 |
|---|---|---|
| `AppSetup` | MonoBehaviour | Composition Root / 全サービスDI / `TtsEngine` enum 定義 |
| `LocalConfigProvider` | Pure C# | config.json 読込（IConfigProvider実装） |
| `CharacterStateController` | MonoBehaviour | 状態管理 / パイプライン制御 / 録音制御 / UserSubtitleUI 参照 |
...
| `XRInteractionUI` | MonoBehaviour | AI字幕 / エラー表示（ボタン色管理は委譲・UICanvas rootに配置） |
| `UserSubtitleUI` | MonoBehaviour | ユーザー発話字幕表示（STT 結果を表示・独立Canvas） |
| `ButtonStateColorController` | MonoBehaviour | ボタンの ON/OFF 状態色と Hover/Press 色の競合のない統合管理 |
| `TeacherPresenter` | MonoBehaviour | VRM / 状態別表示 |
| `ExternalSpeechToTextClient` | Pure C# | STT API通信（Whisper） |
| `ExternalLanguageModelClient` | Pure C# | LLM API通信（1問1答 Stateless 設計） |
| `ExternalElevenLabsClient` | Pure C# | TTS API通信 ElevenLabs（AudioClip返却・`TtsEngine.ElevenLabs` 選択時） |
| `ExternalFishAudioClient` | Pure C# | TTS API通信 Fish Audio（AudioClip返却・`TtsEngine.FishAudio` 選択時） |
| `ExternalOpenAiTtsClient` | Pure C# | TTS API通信 OpenAI（AudioClip返却・`TtsEngine.OpenAI` 選択時） |
| `ExternalGoogleTtsClient` | Pure C# | TTS API通信 Google Cloud TTS（Base64 MP3→一時ファイル→AudioClip・`TtsEngine.Google` 選択時） |
...
- [ ] `UserSubtitleUI.cs` 実装・World Space Canvas に配置
...
- [ ] `RecordButton` に `BoxCollider`（isTrigger=true）・`ColliderSurface`・`PlaneSurface`・`BoundsClipper`・`ClippedPlaneSurface` を追加
- [ ] `RayInteractable`（Surface: ColliderSurface）と `PokeInteractable`（SurfacePatch: ClippedPlaneSurface）を追加
- [ ] `InteractableUnityEventWrapper` を Ray / Poke それぞれ1つずつ追加し、WhenHover/Select から `ButtonStateColorController.External*()` と `CharacterStateController.OnRecordButtonTapped()` を接続
...
- [ ] 使用する TTS クライアント実装・`AppSetup.ttsEngine` で選択
  - ElevenLabs: `ExternalElevenLabsClient.cs`
  - Fish Audio: `ExternalFishAudioClient.cs`
  - Google Cloud TTS: `ExternalGoogleTtsClient.cs`
  - OpenAI TTS: `ExternalOpenAiTtsClient.cs`
- [ ] Editorでエンドツーエンド動作確認
...
本システムは、各機能をインターフェース（`IService`）で抽象化しているため、コアロジックを修正することなく以下の拡張が可能です。

### 設計上の拡張ロードマップ

```mermaid
graph TD
    subgraph "Phase 1: 内部機能の高度化"
        A1["ExternalLanguageModelClient"] -.->|実装差し替え| B2["RagLanguageModelClient\n(知識ベース/FastAPI連携)"]
        C1["TeacherPresenter"] -.->|処理追記| D2["TeacherPresenter\n+ リップシンク (uLipSync)\n+ アニメーター制御"]
        E1["会話履歴保持"] -.->|クライアント内実装| E2["ContextualMemory\n(Token管理/UIリセット)"]
    end
```
```mermaid
graph TD
    subgraph "Phase 2: セキュリティと実運用"
        F1["Unity (Direct API Call)"] ==>|アーキテクチャ変更| F2["中間APIサーバー (FastAPI/Node.js)\nAPIキーの隠蔽・認証"]
    end
```
```mermaid
graph TD
    subgraph "Phase 3: 次世代ユーザー体験"
        G1["直列処理バケツリレー\n(STT → LLM → TTS)"] -.->|パラダイムシフト| G2["双方向ストリーミング\n(OpenAI Realtime API / WebSocket)"]
    end
```

### 重点拡張項目

#### 1. RAG（知識ベース追加による専門性の向上）
* **インパクト:** **大**（特定の専門知識を持つ「先生」としての信頼性が劇的に向上します）
* **実装方針:** `ILanguageModelService` を継承した `RagLanguageModelClient` を新規実装します。LangChainやLlamaIndexを活用した外部API（FastAPI等）と通信させることで、Unity側のフロー制御（`CharacterStateController`）を汚さずに専門知識を注入可能です。

#### 2. リップシンク・アニメーション（プレゼンスの強化）
* **インパクト:** **特大**（キャラクターの「実在感」に直結します）
* **実装方針:** 表示責務を持つ `TeacherPresenter` に集約します。`OVRLipSync` や `uLipSync` を導入し、`Speaking` 状態時にオーディオ波形を解析してブレンドシェイプを制御します。ステート管理と描画ロジックが分離されているため、他のモジュールへの影響はありません。

#### 3. 会話履歴の管理（コンテキストの保持）
* **インパクト:** **中**（文脈を汲み取った自然な対話が可能になります）
* **実装方針:** `LLMClient` 内に `List<Message>` を保持し、リクエストに過去のコンテキストを含めます。これに伴い、「話題のリセット」を通知するUIイベントと、トークン上限に応じた古い履歴の破棄（FIFO）ロジックを実装します。

#### 4. 中間APIサーバー化（セキュリティと運用の最適化）
* **インパクト:** **必須**（商用利用や配布を想定した場合、APIキーの保護は不可欠です）
* **実装方針:** Unityの `config.json` からAPIキーを排除し、FastAPI等で構築したサーバー側に秘匿します。Unity側は各Clientの接続先URLを自社サーバーに向けるだけで対応可能です。これにより、レート制限やログ監視もサーバー側で一元管理できます。

#### 5. 双方向音声ストリーミング（Realtime APIへの移行）
* **インパクト:** **パラダイムシフト**（応答速度と自然さが根本から変わります）
* **実装方針:** 従来の「STT → LLM → TTS」という直列処理から、WebSocketによる常時接続へ移行します。`ISpeechToTextService` と `ITextToSpeechService` の境界が消失し、`CharacterStateController` の状態遷移（Thinking状態の廃止など）を含む大規模なアップデートとなりますが、最も強力なユーザー体験を提供します。

### 拡張機能一覧と変更点

| カテゴリ | 拡張機能 | 実装方法・使用ライブラリ | 主な変更箇所 |
| :--- | :--- | :--- | :--- |
| **知能** | **RAG対応** | `RagLanguageModelClient` の追加 / FastAPI連携 | `AppSetup` (DI設定) |
| | **会話履歴** | `List<Message>` による履歴保持とToken管理 | `LLMClient` |
| **表現** | **リップシンク** | `uLipSync` 等による波形解析 | `TeacherPresenter` |
| | **モーション** | `Animator` の遷移トリガー追加 | `TeacherPresenter` |
| **基盤** | **中間サーバー** | FastAPI等へのリクエスト転送 / キー隠蔽 | 各Clientの接続先URL |
| | **Realtime API** | WebSocketによる双方向ストリーミング | 全サービス・`StateController` |
| | **キャンセル** | `CancellationToken` の伝播 | 全インターフェース |
| **追加** | **STT/TTS追加** | `AzureSpeechToText` 等の新クライアント実装 | `AppSetup` / `TtsEngine` enum |

---

### 補足：アーキテクチャの柔軟性について
現在の設計は、技術的負債を抑えつつ、上記のような「難易度の高い拡張」にも耐えられるよう、各コンポーネントが疎結合に保たれています。特にPhase 3のRealtime APIへの移行は大きな変更ですが、それ以外の項目は既存のインターフェースの枠組みの中で安全に実装を進めることが可能です。
...
- **AppSetup GameObject**
  ...
  - [ ] `useMock`: デバッグ時は `true`、API接続時は `false`
  - [ ] `ttsEngine`: 使用する TTS エンジンを選択（ElevenLabs / FishAudio / Google / OpenAI）
- **CharacterRoot GameObject (CharacterStateController)**
  - [ ] `ui`: シーン内の XRInteractionUI をアタッチ
  - [ ] `userSubtitleUI`: シーン内の UserSubtitleUI をアタッチ（**null でも動作するが表示されない**）
...
- **UICanvas Root (XRInteractionUI)**
  - [ ] `subtitleText`: AI字幕用 TextMeshPro をアタッチ
  - [ ] `buttonLabel`: `RecordButton > ButtonLabel` の TextMeshPro をアタッチ
  - [ ] `_buttonColor`: `RecordButton > ButtonBackground` の `ButtonStateColorController` をアタッチ
- **RecordButton（ISDK ネイティブボタン）**
  - [ ] `BoxCollider`: isTrigger=true、サイズはボタン寸法に合わせて調整
  - [ ] `ColliderSurface._collider`: 同オブジェクトの BoxCollider をアサイン
  - [ ] `ClippedPlaneSurface._planeSurface`: 同オブジェクトの PlaneSurface をアサイン
  - [ ] `ClippedPlaneSurface.Clippers`: BoundsClipper を追加
  - [ ] `RayInteractable.Surface`: ColliderSurface をアサイン
  - [ ] `PokeInteractable.SurfacePatch`: ClippedPlaneSurface をアサイン
  - [ ] `InteractableUnityEventWrapper [Ray用]._interactableView`: RayInteractable をアサイン
  - [ ] `InteractableUnityEventWrapper [Poke用]._interactableView`: PokeInteractable をアサイン
  - [ ] 両 Wrapper の `WhenHover/WhenUnhover/WhenSelect/WhenUnselect` を `ButtonStateColorController.External*()` および `CharacterStateController.OnRecordButtonTapped()` に接続
- **ButtonBackground (ButtonStateColorController)**
  - [ ] 色パラメータ（offNormal / onNormal 等）が設定されていることを確認
- **UserSubtitleUI GameObject**
  - [ ] `userSubtitleText`: ユーザー発話表示用 TextMeshPro をアタッチ
  - [ ] `userTextColor`: デフォルトは黒

---

*設計書バージョン: 5.2 | 更新日: 2026-05-14*
