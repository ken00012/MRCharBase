...
| TTS | ElevenLabs / Fish Audio（切り替え可能） |
...
> ElevenLabs を使用する場合、Fish Audio 関連の3フィールドは省略可能です。
...
| TTS | ElevenLabs API / Fish Audio API（`AppSetup.TtsEngine` で選択） |
| 通信 | UnityWebRequest |
...
│   │   │   │   ├── ExternalTextToSpeechClient.cs  ← ElevenLabs TTS
...
public class AppSetup : MonoBehaviour
{
    ...
    private async UniTaskVoid InitAsync()
    {
        ...
        // TtsEngine.FishAudio を選択した場合は ExternalFishAudioClient を使用
...
| `ExternalTextToSpeechClient` | Pure C# | TTS API通信 ElevenLabs（AudioClip返却・`TtsEngine.ElevenLabs` 選択時に使用） |
