...
| TTS | ElevenLabs / Fish Audio / Google Cloud TTS / OpenAI TTS（Inspectorで切り替え） |
...
└── _MRCharBase/
    ...
    │   │   ├── External/     実際の API クライアント（ElevenLabs / FishAudio / Google TTS / OpenAI TTS）
    │   │   └── Mock/         Mock 実装（削除禁止）
...
| TTS エンジン変更 | `AppSetup` の `TtsEngine` を Inspector で切り替え |
...
- `prototype_design.md` — 詳細設計書（v4.6）

---

*設計書バージョン: prototype_design.md v4.6*
